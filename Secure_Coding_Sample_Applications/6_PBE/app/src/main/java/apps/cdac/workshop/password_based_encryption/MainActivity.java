package apps.cdac.workshop.password_based_encryption;
...
public class MainActivity extends AppCompatActivity {
    ...
    public void onEncryptClick(View view) {
        try {
            ...
            byte[] data = etInput.getText().toString().trim().getBytes(StandardCharsets.UTF_8);
            encData = encrypt(password, data);

            tvOutput.setText(new String(encData.encryptedData, "UTF-8"));
        } catch (Exception e) {
    ...
    public void onDecryptClick(View view) {
        try {
            ...
            tvOutput.setText(decDataAsString);
        } catch (Exception e) {
        }
    }
